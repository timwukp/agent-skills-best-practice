...
| Skill | Description |
|-------|-------------|
| [ai-native-sdlc](skills/ai-native-sdlc/) | **Flagship.** Runs a change as a loop of committed, machine-readable artifacts (intent.md → spec.md → plan.md → diff+tests → PR → bands.yaml → new intent.md), and makes the stage order *enforceable* rather than advisory via a PreToolUse hook (write-time, fails open) and a CI gate (merge-time, fails closed). Ships as a reusable `workflow_call` workflow so consuming repos pin a version instead of vendoring a drifting copy. Its own gates carry 6 test suites verified by mutation testing (36 mutations, 36 killed), signed releases with an SBOM and SLSA provenance, a threat model for the two attack surfaces it introduces, and a `references/limitations.md` stating what a green gate does not mean — including that no gate can judge whether an eval is meaningful. |
