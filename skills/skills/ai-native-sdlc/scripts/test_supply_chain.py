...
def test_release_runs_every_suite() -> None:
    ...
    # Tests run before packaging and create scripts/__pycache__/*.pyc. Packaging the
    # live checkout therefore includes generated bytecode that is absent from the
    # file-level SBOM. This was not theoretical: the published sdlc-gate-v2 tar had
    # five .pyc files omitted by the SBOM, while the SBOM named two eval files absent
    # from the tar. Stage from Git's tracked tree so test residue cannot enter a signed
    # release, then inventory that exact staged tree rather than the larger checkout.
    check(
        "release stages tracked bytes instead of packaging the test-mutated checkout",
    ...
    check(
        ...
        "inventorying $SKILL_DIR includes files the tar omits and misses generated files it includes",
    )
    check(
        "release proves artifact and SBOM file inventories are identical",
        'diff -u "$ARTIFACT_FILES" "$SBOM_FILES"' in text,
        "a signed artifact with an incomplete or over-broad SBOM is not consumable evidence",
    )
    check(
